...
def detailed_benchmark():
    ...
    # Test 2: NumPy only
    numpy_data = {"array": np.random.rand(1000), "metadata": {"size": 1000}}
    ...
    pydantic_data = [
        User(id=i, name=f"user_{i}", email=f"user_{i}@test.com") for i in range(1000)
    ]
    ...
    for test_name, test_data in tests:
        ...
        print(
            f"   Speedup: {json_time/bfast_time:>6.1f}x vs JSON, {orjson_time/bfast_time:>6.1f}x vs orjson"
        )
        ...
        print(
            f"   Size:    JSON {json_size}b, B-FAST {bfast_size}b ({(1-bfast_size/json_size)*100:.1f}% smaller)"
        )
...
def micro_benchmark():
    ...
    for test_name, test_data in micro_tests:
        ...
        for _ in range(iterations):
            bf_encoder.encode_packed(test_data, compress=False)
        bfast_time = (
            (time.perf_counter() - start) / iterations * 1000000
        )  # microseconds
        ...
        print(
            f"{test_name:>12}: B-FAST {bfast_time:>6.1f}μs, JSON {json_time:>6.1f}μs, Ratio {json_time/bfast_time:>4.1f}x"
        )
