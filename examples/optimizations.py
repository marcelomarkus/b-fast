...
def example_compression():
    ...
    print(f"  Without compression: {time_uncompressed:6.2f}ms | {len(uncompressed):,} bytes")
    print(f"  With compression:    {time_compressed:6.2f}ms | {len(compressed):,} bytes")
    ...
    print(f"✅ Minimal overhead thanks to parallelism\n")
...
def example_network_scenarios():
    ...
    print(f"{'Network':<15} {'Without Compression':<20} {'With Compression':<20} {'Best'}")
    print("-" * 70)
    ...
    for name, bps in networks:
        ...
        better = "✅ Compressed" if total_compressed < total_uncompressed else "✅ Normal"
        speedup = total_uncompressed / total_compressed
        
        print(f"{name:<15} {total_uncompressed:>6.1f}ms           "
              f"{total_compressed:>6.1f}ms ({speedup:.1f}x)    {better}")
    
    print(f"\n💡 Recommendations:")
    print(f"  • Slow networks (< 100 Mbps): compress=True")
    print(f"  • Fast networks (> 1 Gbps): compress=False")
    print(f"  • Mobile/IoT: always compress=True\n")
...
def example_encoder_reuse():
    ...
    print(f"✅ Ideal for serializing multiple batches\n")
...
def example_numpy():
    ...
    try:
        ...
        print(f"✅ Ideal for ML/Data Science pipelines\n")
