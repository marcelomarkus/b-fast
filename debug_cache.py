...
def debug_pydantic_cache():
    ...
    user = User(id=1, name="Test", email="test@test.com", active=True, scores=[1.0, 2.0, 3.0])
    ...
    for i in range(1000):
        ...
        result = bf_encoder.encode_packed(user, False)
    ...
    for i in range(100):
        start = time.perf_counter()
        result = bf_encoder.encode_packed(users_10, False)
        end = time.perf_counter()
    ...
    for i in range(10):
        start = time.perf_counter()
        result = bf_encoder.encode_packed(users_100_diff, False)
        end = time.perf_counter()
        times.append((end - start) * 1000)
    ...
    for i in range(10):
        start = time.perf_counter()
        result = bf_encoder.encode_packed(users_100_same, False)
        end = time.perf_counter()
        times.append((end - start) * 1000)
