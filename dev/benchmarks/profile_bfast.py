...
def profile_bfast():
    ...
    print(
        f"Single user (1000x): {(end-start)*1000:.2f}ms total, {(end-start):.3f}ms per call"
    )

    # Test list of primitives
    primitives = list(range(1000))
    start = time.perf_counter()
    for _ in range(10):
        bf_encoder.encode_packed(primitives, False)
    end = time.perf_counter()
    print(
        f"1000 integers (10x): {(end-start)*1000:.2f}ms total, {(end-start)*100:.2f}ms per call"
    )
