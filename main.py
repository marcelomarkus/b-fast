...
def benchmark_serializers():
    ...
    # Dados de teste MAIORES (onde B-FAST brilha)
    users = [
        User(id=i, name=f"User {i}", email=f"user{i}@test.com", 
             active=i % 2 == 0, scores=[float(i), float(i*2), float(i*3)])
        for i in range(10000)  # 10x mais dados
    ]
    
    # Dados repetitivos (string interning advantage)
    repeated_data = {
        "users": users,
        "metadata": {"version": "1.0.5", "timestamp": time.time(), "status": "active"},
        "config": {"debug": True, "timeout": 30, "retries": 3},
        "numpy_array": np.random.rand(1000)  # Array maior
    }
    ...
    def test_bfast():
        return bf_encoder.encode_packed(repeated_data, compress=False)
    
    def test_bfast_compressed():
        return bf_encoder.encode_packed(repeated_data, compress=True)
    ...
    print(f"{'Método':<15} {'Tempo (ms)':<12} {'Speedup':<10} {'Tamanho':<12} {'Redução':<10}")
    print("-" * 65)
    ...
    for name, avg_time, avg_size in results:
        ...
        print(f"{name:<15} {avg_time:>8.2f}ms   {speedup:>6.1f}x     {avg_size:>8.0f}b   {size_reduction:>6.1f}%")
    ...
    print(f"   • {(1 - bfast_compressed[2]/json_size)*100:.1f}% menor payload (com LZ4)")
    print(f"   • Zero-copy NumPy arrays")
    print(f"   • Pydantic native (sem .model_dump())")
